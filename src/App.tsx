...
export default function App() {
  ...
  const handleNavigateToQuiz = (moduleId: string) => {
    ...
    setTimeout(() => {
      const event = new CustomEvent("selectQuiz", {
        detail: { quizId: moduleId },
      });
      window.dispatchEvent(event);
    }, 100);
  };
  ...
  const renderActiveTab = () => {
    switch (activeTab) {
      case "home":
        return (
          <HomeTab
            onNavigateToModule={handleQuickGuideClick}
  ...
  return (
    <ProgressProvider>
      <div className="h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-black flex flex-col">
        {/* Main Content */}
        <div className="flex-1 overflow-auto pb-20">{renderActiveTab()}</div>
                ...
                <button
                  ...
                  className={`relative flex flex-col items-center transition-all duration-300 ${
                    isCenter
                      ...
                      : "py-2 px-3"
                  } ${isActive && !isCenter ? "transform scale-105" : ""}`}
                >
                  <div
